...
class UserFileList extends Component {
  ...
  getFiles = async () => {
    ...
    for (let j = 0; j < this.files.length; j++) {
      ...
      let date = this.props.dateFormatter(
        new Date(this.files[j].timeStamp * 1000)
      );
      ...
      this.table.push(
        <Table.Row>
          ...
          <Table.Cell>
            <a href={hrefValue}>{this.files[j].hash}</a> click
          </Table.Cell>
          <Table.Cell>{date}</Table.Cell>
          <Table.Cell>
            <NavLink to={signHref}>
              <Button color='blue'>Sign Link</Button>
            </NavLink>
          </Table.Cell>
        </Table.Row>
      );
  ...
  getNumberOfUserFiles = async () => {
    const flag = await fileAuthorContract.methods.getNumberOfUserFiles().call({
  ...
  getUserFile = async index => {
    const file = await fileAuthorContract.methods.getUserFile(index).call({
      from: this.accounts[0]
    });
    return {
      hash: file.fileHash,
  ...
  render() {
    return (
      <div>
        <Button color='teal' onClick={this.getFiles}>
          Show User Files List
        </Button>
            ...
            <Table.Header>
              <Table.Row>
                <Table.HeaderCell colSpan='3'>
                  <div style={{ color: '#EE694C' }}>{this.accounts[0]}</div>{' '}
                  ACCOUNT HOLDER Files
                </Table.HeaderCell>
              </Table.Row>
            </Table.Header>
